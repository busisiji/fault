...
"""Program which detects USB mySerial ports.
This program will search for a USB mySerial port using a search criteria.
In its simplest form, you can use the -l (--list) option to list all of
the detected mySerial ports.
...
--seral  00123  Will only match devices whose mySerial number stats with 00123
...
where you want to pass the name of the mySerial port into a utiity to open.
"""
...
def extra_info(device):
    ...
    if 'ID_SERIAL_SHORT' in device.properties:
        extra_items.append("mySerial '%s'" % device.properties['ID_SERIAL_SHORT'])
    if extra_items:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Find the /dev/tty port for a USB mySerial devices",
    )
    parser.add_argument(
        "-l", "--list",
        dest="list",
        action="store_true",
        help="List USB mySerial devices currently connected"
    )
    parser.add_argument(
        "-s", "--mySerial",
        dest="mySerial",
        help="Only show devices with the indicated mySerial number",
        default=None,
    )
    parser.add_argument(
        "-n", "--vendor",
    ...
    if args.list:
        '''Print all USB mySerial devices'''
        devices = list_devices(**vars(args))
        for device in devices:
            print('USB mySerial Device {}:{}{} found @{}'.format(*device))
        if len(devices) == 0:
            print('No USB mySerial devices detected.')
        return
    ...
    # sys.exit(1)
    raise Exception("No USB mySerial devices detected.")
